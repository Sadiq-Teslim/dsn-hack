...
function humanTraceNotes(step) {
  ...
  if (name === "ResolveUserProfileStep") {
    notes.push("I reviewed the selected persona and the user's past preferences.");
    ...
    if (outputs.nigerian_register) notes.push(`I kept the user's preferred communication style as ${outputs.nigerian_register}.`);
    if (outputs.top_taste_tokens?.length || outputs.taste_tokens?.length) {
      notes.push(`I noted interests such as ${friendlyList(outputs.top_taste_tokens || outputs.taste_tokens, 4)}.`);
    }
    return notes;
  }

  if (name === "RetrieveEvidenceStep") {
    notes.push(`I selected ${plural(outputs.evidence_count, "reference review")} to guide the generated review.`);
  ...
  if (name === "PredictSentimentStep") {
    notes.push(`I estimated a ${reactionLabel(outputs.sentiment)} reaction, about ${formatPercent(outputs.sentiment)} on the reaction scale.`);
  ...
  if (name === "CalibrateRatingStep") {
    if (isNumber(outputs.raw_rating) && isNumber(outputs.calibrated_rating)) {
      notes.push(`I moved the rating from ${formatNumber(outputs.raw_rating, 1)}/5 to ${formatNumber(outputs.calibrated_rating, 1)}/5 after applying the user's rating style.`);
    } else {
      notes.push(`I chose a ${outputs.calibrated_rating}/5 rating after comparing the product fit with the user's usual rating style.`);
    }
    return notes;
  }

  if (name === "GenerateReviewStep") {
    notes.push("I wrote the review in the user's likely tone.");
    if (outputs.fallback_used === false) notes.push("I used the hosted language service for the final wording.");
    if (outputs.fallback_used === true) notes.push("I used the local backup writer for this result.");
    return notes;
  }

  if (name === "ConsistencyCheckStep") {
    notes.push(cleanTraceSentence(outputs.consistency_check || "I checked that the review and rating agree."));
    return notes;
  }

  if (name === "FormalizeReviewReasoningStep" || name === "BuildRecommendationResponseStep") {
    notes.push("I rewrote the visible explanation so the result reads clearly for a person reviewing it.");
    if (isNumber(outputs.item_count)) notes.push(`The final response contains ${plural(outputs.item_count, "recommendation")}.`);
    if (outputs.presentation_llm_configured === true) notes.push("I used the hosted language service for the final explanation.");
    if (outputs.presentation_fallback_used === true) notes.push("I used a local backup explanation because the hosted service was unavailable.");
    return notes;
  }
  ...
  if (name === "CrossDomainBridgeStep") {
    if (outputs.is_cross_domain) notes.push("I connected the user's interests from one area to another before recommending.");
    if (outputs.taste_descriptors?.length) notes.push(`I used ${plural(outputs.taste_descriptors.length, "preference idea")}, including ${friendlyList(outputs.taste_descriptors, 5)}.`);
    if (!notes.length) notes.push("I checked whether the request needed ideas to be connected across categories.");
    return notes;
  }

  if (name === "ColdStartGateStep") {
    if (outputs.follow_up_questions?.length) {
      notes.push("I decided that more information was needed before making a recommendation.");
      notes.push(`I asked: ${friendlyList(outputs.follow_up_questions, 2)}.`);
    } else {
      notes.push("I had enough information to continue with the recommendation.");
    }
    return notes;
  }
  ...
  if (name === "LLMRerankStep") {
    notes.push("I compared the remaining options and placed the strongest matches first.");
    if (outputs.top_reranked?.length) notes.push(`I reported ${plural(outputs.top_reranked.length, "leading choice")}: ${friendlyList(outputs.top_reranked, 4)}.`);
    if (outputs.fallback_used === true) notes.push("I used the local backup comparison for this step.");
    return notes;
  }

  if (name === "DiversityStep") {
    notes.push("I checked the list so the final result was not too repetitive.");
    if (outputs.selected_categories?.length) {
      const uniqueCount = new Set(outputs.selected_categories).size;
      notes.push(`I kept the final mix to ${plural(uniqueCount, "category")}: ${friendlyCategories(outputs.selected_categories)}.`);
    }
    return notes;
  }
...
function cleanTraceSentence(text) {
  const value = String(text || "")
    .replace(/^passed:\s*/i, "I confirmed that ")
    .replace(/^warning:\s*/i, "I noted that ")
    .replaceAll("_", " ")
    .trim();
  return value ? value[0].toUpperCase() + value.slice(1) : "I completed this check.";
}
